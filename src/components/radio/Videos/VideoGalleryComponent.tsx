...
const VideoGalleryComponent: React.FC = () => {
  ...
  return (
    <div className="w-full px-4 py-12">
      <h2 className="text-5xl font-extrabold text-red-600 mb-10 text-center">Galeria de Vídeos</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
        {videos.map((video, index) => (
          <div key={index} className="relative group rounded-lg shadow-2xl overflow-hidden transform hover:scale-105 transition-transform duration-500">
            ...
            <div className="p-4">
              <h3 className="text-xl font-bold text-gray-800 group-hover:text-red-600 transition-colors duration-300">{video.title}</h3>
            </div>
            <div className="absolute -bottom-5 -left-5 w-16 h-16 bg-red-300 rounded-full blur-2xl opacity-50"></div>
            <div className="absolute -top-5 -right-5 w-20 h-20 bg-red-400 rounded-full blur-3xl opacity-40"></div>
          </div>
        ))}
      </div>
    </div>
