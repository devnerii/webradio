...
const PhotoGalleryComponent: React.FC = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <h2 className="text-5xl font-extrabold text-blue-600 mb-10 text-center">Galeria de Fotos</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        {photos.map((photo, index) => (
          <div key={index} className="relative group rounded-lg shadow-2xl overflow-hidden transform hover:scale-105 transition-transform duration-500">
            ...
            <div className="p-6">
              <h3 className="text-2xl font-bold text-gray-800 group-hover:text-blue-600 transition-colors duration-300">{photo.title}</h3>
            ...
            <div className="absolute -top-5 -right-5 w-24 h-24 bg-blue-400 rounded-full blur-3xl opacity-40"></div>
          </div>
        ))}
      </div>
    </div>
